...
model SingleZoneResidentialHydronicHeatPump
  ...
  IDEAS.Fluid.HeatExchangers.RadiantSlab.EmbeddedPipe floHea(
    ...
    annotation (Placement(visible = true, transformation(origin = {0, 0}, extent = {{0, 0}, {-20, 20}}, rotation = 0)));
  ...
  Modelica.Blocks.Interfaces.RealInput yOcc annotation(
    Placement(visible = true, transformation(origin = {-78, 42}, extent = {{-12, -12}, {12, 12}}, rotation = 0), iconTransformation(origin = {-86, 40}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));
equation
  ...
  connect(case900Template.gainEmb[1], floHea.heatPortEmb[1]) annotation(
    Line(points = {{-60, 1}, {-40, 1}, {-40, 20}, {-10, 20}}, color = {191, 0, 0}, pattern = LinePattern.Solid));
  connect(pum.port_b, floHea.port_a) annotation(
    Line(points = {{20, 40}, {0, 40}, {0, 10}}, color = {0, 127, 255}, pattern = LinePattern.Solid));
  ...
  connect(floHea.QTot, reaQFloHea.u) annotation(
    Line(points = {{-21, 16}, {-32, 16}, {-32, -50}, {-22, -50}}, color = {0, 0, 127}, pattern = LinePattern.Solid));
  ...
  connect(senTemRet.port_a, floHea.port_b) annotation(
    Line(points = {{60, -20}, {-20, -20}, {-20, 10}}, color = {0, 127, 255}, pattern = LinePattern.Solid));
  connect(yOcc, case900Template.yOcc) annotation(
    Line(points = {{-78, 42}, {-54, 42}, {-54, 13}, {-58, 13}, {-58, 14}}, color = {0, 0, 127}, pattern = LinePattern.Solid));
  connect(yOcc, greater.u1) annotation(
    Line(points = {{-78, 42}, {-78, 61}, {-82, 61}, {-82, 80}}, color = {0, 0, 127}, pattern = LinePattern.Solid));
  annotation (
    experiment(
      StopTime=1728000,
